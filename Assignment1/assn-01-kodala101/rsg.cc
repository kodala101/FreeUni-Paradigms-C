...
void recursion(map<string, Definition>& grammar, string word) {
  const Production& random = grammar[word].getRandomProduction();
  for (auto curr = random.begin(); curr != random.end(); ++curr) {
    string s = *curr;
    if (s[0] == '<') {
      recursion(grammar, s);
    } else {
...
void generateSentence(map<string, Definition>& grammar) {
   const Production& startProd = grammar["<start>"].getRandomProduction();
   for (auto curr = startProd.begin(); curr != startProd.end(); ++curr) {
    string word = *curr;
    if (word[0] == '<') {
      recursion(grammar, word);
    } else {
      cout << word << " ";
    }
   }
