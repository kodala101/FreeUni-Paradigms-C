...
(define (quicksort num-list)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car(car num-list)) (cdr num-list))))
...
(define (intersection-points-helper l) 
  (apply append (map (lambda (e) (intersect (car l) e)) (cdr l)))
)

(define (intersection-points l)
  (if (null? l) '() 
    (append (intersection-points-helper l) (intersection-points (cdr l)))
  )
)

(define (distance-product p l)
  (apply * (map (lambda (e) (if (= (dist p e) 0) 1 (dist p e))) l))
)

(define (rate-points l)
  (map (lambda (e) (list (distance-product e l) e)) l)
)
...
(define (clumped-points-helper l len)
  (if (= 0 len) '() (cons (car l) (clumped-points-helper (cdr l) (- len 1))))
)

(define (clumped-points l)
  (let* ((sorted (sort-points (rate-points l))) (sortedPointsOnly (map cadr sorted)))
    (if (even? (length sorted)) (clumped-points-helper sortedPointsOnly (quotient (length sorted) 2)) 
    (clumped-points-helper sortedPointsOnly (quotient (- (length sorted) 1) 2)))
  )
)

(define (average-point-helper l)
  (list (/ (apply + (map car l)) (length l)) (/ (apply + (map cadr l)) (length l)))
)

(define (average-point l)
  (let ((avg (average-point-helper l)))
    (list (distance-product avg l) avg)
  )
)
...
(define (where-am-i distances starLocations)
  (sort-points (map best-estimate (all-guesses distances starLocations)))
)
