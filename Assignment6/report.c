...
int Report_Transfer(Bank *bank, int workerNum, AccountNumber accountNum,
  ...
  sem_wait(&(bank->reportTransferLock));  //
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&(bank->reportTransferLock));  //
...
int Report_DoReport(Bank *bank, int workerNum) {  //!!!!!!!!
  sem_wait(&(bank->check));
  bank->notDoneWorkers--;
  if (bank->notDoneWorkers == 0) {
    Report *rpt = bank->report;

    assert(rpt); Y;

    if (rpt->numReports >= MAX_NUM_REPORTS) {
        bank->notDoneWorkers = numWorkers;
        sem_post(&(bank->check));
        for (int i = 0; i < numWorkers - 1; i++) sem_post(&(bank->readyForNextDay));
        // We've run out of report storage for the bank
        return -1;
    }
